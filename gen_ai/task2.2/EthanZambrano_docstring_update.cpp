...
* Description: The main function drives the parallel long addition program.
* Operation:
*   - Prompts the user to enter two valid large numbers.
*   - Converts the input strings into integer vectors (`num1` and `num2`).
*   - Ensures both numbers are of equal length by padding `num2` with leading zeros.
*   - Determines the number of threads based on available hardware concurrency,
*     reducing to the nearest power of 2.
*   - Spawns threads to execute `parallelLongAddition` on different segments of the numbers.
*   - Waits for all threads to complete.
*   - Handles carry-over propagation from right to left.
...
* Description: Performs parallelized long addition on a segment of the input numbers.
* Operation:
*   - Iterates through the assigned segment of digits.
*   - Computes the sum of corresponding digits from `num1` and `num2`.
*   - If the sum exceeds 9, stores the carry in the `carryOver` vector.
*   - Stores the result in the `resultSum` vector.
